  ...
  mutate(indigena_es = case_when(a1 >= 10 ~ "No indígena",
                                 a1 <= 12   ~ "Indígena")) %>% 
                                  select(edad,
                                         ...
                                         d1_3, #Conflicto con las empresas.
                                         d2_1,
                                         d2_3,
                                         c1,
...
mutate(across(c(d1_1, d1_2, d1_3, d2_1, d2_3, #Conflicto
                c1, c2, c4, c5, c23, #Intergrupal
                c7_2, c7_3, c13, c14,
                 c35, c36, a6, a4, ), 
              ~as.numeric(.)))
...
model_conflict <- fit_logistic(elri_prep, d14 ~ mujer + edad + urbano_rural +
                                 d1_1 + d1_2 + d1_3 + cerca_conflicto)
...
# Añadir una columna para identificar el modelo y combinar los resultados
results_identity <- results_identity %>% mutate(Modelo = "Modelo de Identidad y Confianza")
results_conflict <- results_conflict %>% mutate(Modelo = "Modelo de Conflicto")
results_contact <- results_contact %>% mutate(Modelo = "Modelo de Contacto Intergrupal")

# Unir todos los modelos en un solo dataframe
results_summary <- bind_rows(results_identity, results_conflict, results_contact)

# Seleccionar y reordenar las columnas de interés
results_summary <- results_summary %>%
  select(Modelo, term, estimate, odds_ratio, p.value) %>%
  arrange(Modelo, term)
